...
def select(Launch_site):
    if Launch_site == 'All Sites':
        ...
        new_df = spacex_df[spacex_df["Launch Site"] == Launch_site]["class"].value_counts().to_frame()
        new_df["name"] = ["Failure", "Success"]
        fig = px.pie(new_df, values='class', names='name', title='Total Success Launches for ' + Launch_site)
    return fig
